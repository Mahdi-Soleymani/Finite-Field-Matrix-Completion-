...
function [error]=MyAlgorithm_boolean_multiplication_picked_by_entropy(p_obs)
...
for rep=1:repeat
    ...
    for i=1:1:50
        ...
        for e=1:1:m
            p=L_b(:,e);
            new_entropy=wentropy(p,'shannon');
            if new_entropy>0 && new_entropy<L_min_entropy
                L_min_entropy=new_entropy;
                L_min_entropy_id=e;
            end
        end
        ...
        for e=1:1:n
            p=R_b(:,e);
            new_entropy= wentropy(p,'shannon');
            if new_entropy>0 && new_entropy<R_min_entropy
                R_min_entropy=new_entropy;
                R_min_entropy_id=e;
            end
        end
%         
...
%         if true
        ...
        if L_min_entropy<R_min_entropy 
            
            vector_to_sample=L_b(:,L_min_entropy_id);
            L_fixed_vals_id=[L_fixed_vals_id,L_min_entropy_id];
            fixed_value=discretesample(vector_to_sample, 1);
            L_b_fixed_vals=[L_b_fixed_vals,double(1:2^r == fixed_value)'];
        else
            
            vector_to_sample=R_b(:,R_min_entropy_id);
            R_fixed_vals_id=[R_fixed_vals_id, R_min_entropy_id];
            fixed_value=discretesample(vector_to_sample, 1);
        ...
        %%% checks whether we have enough non-zero elements
